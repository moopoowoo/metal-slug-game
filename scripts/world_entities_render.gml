...
with(Player)
{
    ...
    for(var i = 0; i < 4; i += 1)
    {
        ...
        //draw_line(x, y+_yo, legX[i], legY[i]+_yo);
        {
            ...
            {
                draw_sprite_ext(sprPlayerArm, _frame, x+lengthdir_x(j, _pdir), y+lengthdir_y(j, _pdir)+_yo, 1, 1, _pdir, c_white, 1);
                _frame += 1;
            }
        }
    }
    ...
    // draw tank
    draw_sprite(sprite_index, image_index, x, y+_yo);
...
with(Cannon)
{
    draw_sprite_ext(sprite_index, image_index, x, y+_yo, image_xscale, image_yscale, direction, image_blend, image_alpha);
}
